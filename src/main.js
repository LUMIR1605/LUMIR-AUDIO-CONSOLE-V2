...
const renderAudioCore = now => {
  ...
  physics.update({
    ...
    // Keep the side channels independent: local bass dominates each woofer,
    // while the shared sub band only supplies its physical foundation.
    leftWoofer: clamp((frame.leftBass * .82 + frame.sub * .18 + frame.kickEnergy * .08) * tuning.wooferForce),
    rightWoofer: clamp((frame.rightBass * .82 + frame.sub * .18 + frame.kickEnergy * .08) * tuning.wooferForce),
    leftLower: clamp((frame.leftBass * .36 + frame.leftMid * .42 + frame.sub * .22) * tuning.wooferForce * .52),
    rightLower: clamp((frame.rightBass * .36 + frame.rightMid * .42 + frame.sub * .22) * tuning.wooferForce * .52),
    leftMid: clamp((frame.leftMid * .72 + frame.mid * .28) * tuning.midForce),
    rightMid: clamp((frame.rightMid * .72 + frame.mid * .28) * tuning.midForce),
    leftHigh: clamp(frame.leftHigh * .24), rightHigh: clamp(frame.rightHigh * .24)
  }, deltaMs);
